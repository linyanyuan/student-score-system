...
assert.match(source, /const\s+gradeOptions\s*=\s*Array\.from\([\s\S]*?\)\.sort\(compareGrade\)/)
assert.match(source, /activeGradeFilter/)
assert.match(source, /setGradeFilter/)
assert.match(source, /setGradeFilter\(''\)/)
assert.match(source, /options=\{gradeOptions\.map/)
assert.match(source, /const\s+matchesGrade\s*=\s*!activeGradeFilter\s*\|\|\s*item\.grade\s*===\s*activeGradeFilter/)
assert.match(source, /return\s+matchesGrade\s*&&\s*matchesKeyword/)
